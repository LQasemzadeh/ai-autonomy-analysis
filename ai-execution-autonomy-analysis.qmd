...
knitr::kable(
  ...
  caption = "Table 5. Descriptive summary of completion time (in seconds) across autonomy conditions after exclusion of one extreme outlier."
)
```
...
Because completion time was compared across three autonomy conditions and the distribution remained slightly right-skewed, a non-parametric Kruskal–Wallis test was used to examine whether completion time differed significantly between conditions.
...
However, the results also show that user involvement remains important even under higher autonomy. Intervention did not disappear; rather, its frequency decreased and its form changed. This implies that designers should not assume that execution autonomy makes user control unnecessary. Instead, systems should be designed to support appropriate supervisory control. Even when the system executes actions directly, users should still be able to inspect outputs, understand what has been generated, and intervene when necessary.
This leads to a more differentiated design implication: higher autonomy appears especially useful when task rules are stable, the cost of system error is manageable, and output validity can be checked systematically. In contrast, stronger user control is likely to remain important when tasks are ambiguous, preference sensitive, high stakes, or difficult to evaluate automatically. Under such conditions, full execution autonomy may reduce effort, but it may also increase the need for transparency, reviewability, and correction mechanisms.
The comparison between Assistance and Execution also suggests that full execution autonomy is not always required to achieve meaningful performance gains. In the present study, the move from Manual to system-supported conditions produced the strongest improvements, whereas the differences between Assistance and Execution were often smaller and not always statistically significant. From a design perspective, this means that assistance-oriented systems may already offer substantial value while preserving a greater degree of user agency. In many practical settings, the most effective design may therefore be neither full manual control nor unconditional full execution, but a calibrated level of autonomy that matches the structure, risk, and reversibility of the task.
More concretely, AI execution autonomy appears most appropriate when:
* the workflow is repetitive and well structured,
* valid outputs can be defined through clear rules or constraints,
* rapid completion is important,
* and users retain the ability to inspect and correct system outputs when needed.
By contrast, stronger user control appears especially important when:
* tasks involve personal judgment or individual preference,
* system outputs are difficult to verify automatically,
* the consequences of incorrect execution are high,
* or accountability and explainability are particularly important.
...
## Conclusion
This study examined how different levels of AI execution autonomy affect user task performance and intervention behavior in a controlled digital workflow. The findings showed that higher execution autonomy was associated with improved task performance, particularly in terms of shorter completion times and lower error occurrence relative to the Manual condition. At the same time, user intervention was not removed entirely. Although intervention occurrence remained similarly high across conditions, intervention count was significantly lower in the system-supported conditions, and the form of intervention changed depending on the level of autonomy.
Overall, the results suggest that AI execution autonomy can improve efficiency and reduce manual burden in structured workflows, but it does not eliminate the need for human oversight. Rather than replacing user control completely, higher autonomy appears to shift the user role toward supervision, correction, and selective intervention. These findings contribute to a more behaviorally grounded understanding of human–AI interaction and highlight the importance of calibrating autonomy to the structure and demands of the task.
