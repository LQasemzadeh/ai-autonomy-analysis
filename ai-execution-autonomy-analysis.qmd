...
# Classify OVERRIDE sub-types in Execution to distinguish
# skip/regeneration behavior from other supervisory overrides
execution_override_types <- data_clean |>
  dplyr::filter(mode == "Execution", action == "OVERRIDE") |>
  dplyr::mutate(override_action = sapply(details, function(d) {
    tryCatch(jsonlite::fromJSON(d)$action, error = function(e) NA_character_)
  })) |>
  dplyr::group_by(session_id) |>
  dplyr::summarise(
    had_skip_exec        = any(override_action == "skip_execution_preview", na.rm = TRUE),
    had_nonskip_override = any(override_action != "skip_execution_preview", na.rm = TRUE),
    .groups = "drop"
  )

# Behavioral recoding:
#   Direct Editing       = FIELD_EDIT in Manual/Assistance; N/A in Execution
#   Suggestion Rejection = AI_SUGGESTION_REJECTED in Manual/Assistance;
#                          skip_execution_preview OVERRIDE in Execution
#   Override = OVERRIDE (non-skip) across all conditions
intervention_types_raw <- data_clean |>
  dplyr::group_by(session_id, mode) |>
  dplyr::summarise(
    started          = any(action == "TASK_STARTED"),
    had_edit_raw     = any(action == "FIELD_EDIT"),
    had_override_raw = any(action == "OVERRIDE"),
    had_reject_raw   = any(action == "AI_SUGGESTION_REJECTED"),
    .groups = "drop"
  ) |>
  dplyr::filter(started) |>
  dplyr::left_join(execution_override_types, by = "session_id") |>
  dplyr::mutate(
    mode = factor(mode, levels = c("Manual", "Assistance", "Execution")),
    had_edit = dplyr::if_else(mode == "Execution", NA, had_edit_raw),
    had_reject = dplyr::case_when(
      mode == "Execution" ~ dplyr::coalesce(had_skip_exec, FALSE),
      TRUE                ~ had_reject_raw
    ),
    had_override = dplyr::case_when(
      mode == "Execution" ~ dplyr::coalesce(had_nonskip_override, FALSE),
      TRUE                ~ had_override_raw
    )
  ) |>
  dplyr::select(session_id, mode, started, had_edit, had_override, had_reject)
...
#| echo: false
#| fig-cap: "Proportion of started sessions containing each behavioral intervention type across autonomy conditions. Intervention types are defined by what users were structurally able to do in each condition. Direct Editing was not available in the Execution condition (N/A)."
#| fig-width: 7
#| fig-height: 4.5

plot_data_it <- intervention_types_data |>
  dplyr::filter(!is.na(proportion)) |>
  dplyr::filter(!(mode == "Execution" & intervention_type == "Override"))
