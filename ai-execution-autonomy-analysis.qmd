...
<a href="https://lqasemzadeh.github.io/ai-autonomy-dashboard/" target="_blank"
...
# Intervention type breakdown — consistent with 01_load_data.R:
#   Direct Editing       = FIELD_EDIT > 0  (N/A for Execution: mandatory workflow step)
#   Override             = OVERRIDE > 0
#   Suggestion Rejection = AI_SUGGESTION_REJECTED > 0
intervention_types_raw <- data_clean |>
  dplyr::group_by(session_id, mode) |>
  dplyr::summarise(
    started          = any(action == "TASK_STARTED"),
    had_edit         = any(action == "FIELD_EDIT"),
    had_override_raw = any(action == "OVERRIDE"),
    had_reject_raw   = any(action == "AI_SUGGESTION_REJECTED"),
    .groups = "drop"
  ) |>
  dplyr::filter(started) |>
  dplyr::mutate(
    mode         = factor(mode, levels = c("Manual", "Assistance", "Execution")),
    had_edit     = dplyr::if_else(mode == "Execution", NA, had_edit),
    # In Execution, OVERRIDE is the only intervention mechanism —
    # it covers both Override and Suggestion Rejection semantics
    had_override = had_override_raw,
    had_reject   = dplyr::if_else(mode == "Execution", had_override_raw, had_reject_raw)
  )
...
intervention_types_data <- intervention_types_raw |>
  ...
  dplyr::mutate(
    intervention_type = dplyr::case_match(
      intervention_type,
      "had_edit"     ~ "Direct Editing",
      "had_override" ~ "Override",
      "had_reject"   ~ "Suggestion Rejection"
    )
  ) |>
  ...
  dplyr::mutate(
    proportion = dplyr::if_else(is.nan(proportion), NA_real_, proportion),
    label      = dplyr::if_else(is.na(proportion), "N/A", paste0(round(proportion * 100), "%"))
  )
```
...
#| echo: false
#| fig-cap: "Proportion of started sessions containing each behavioral intervention type across autonomy conditions. Direct Editing was not available in the Execution condition (N/A). In the Execution condition, Override includes follow-up adjustments after suggestion regeneration."
#| fig-width: 7
#| fig-height: 4.5

plot_data_it <- intervention_types_data |>
  dplyr::filter(!is.na(proportion), proportion > 0)
na_data_it   <- intervention_types_data |> dplyr::filter(is.na(proportion))

ggplot(plot_data_it, aes(x = intervention_type, y = proportion, fill = mode)) +
  geom_col(position = position_dodge(width = 0.75), width = 0.65) +
  geom_text(
    aes(label = label),
    position = position_dodge(width = 0.75),
  ...
  geom_text(
    data = na_data_it,
    aes(x = intervention_type, label = paste0(mode, ":\nN/A")),
    y = 0.04,
    color = "#FC8D62",
    size = 3,
    fontface = "italic",
  ...
  labs(
    x = "Behavioral intervention type",
    y = "Proportion of sessions",
...
Override occurred in the Assistance (81%) and Execution (91%) conditions and was absent in the Manual condition. In the Assistance condition, override reflected explicit user modification of AI-generated suggestions. In the Execution condition, override additionally included follow-up adjustments after suggestion regeneration, such as reselecting the semester after skipping a previous suggestion.

Suggestion Rejection / Regeneration was present in both the Assistance condition (35%, via the reject action) and the Execution condition (91%, via skip/regeneration), and was absent in the Manual condition.

Overall, the behavioral pattern indicates that intervention remained common across autonomy conditions, but differed in form depending on the degree of AI execution autonomy. While Manual and Assistance relied strongly on direct editing, Execution interventions were concentrated around regeneration and follow-up adjustments after AI-generated suggestions.

```{r}
#| label: intervention-types-fisher
#| echo: false

# Fisher's exact test for Override (all three conditions)
...
# Fisher's exact test for Suggestion Rejection (all three conditions;
# Execution uses skip_execution_preview, Assistance uses AI_SUGGESTION_REJECTED)
reject_table <- table(intervention_types_raw$mode, intervention_types_raw$had_reject)
fisher_reject <- fisher.test(reject_table)

# Fisher's exact test for Direct Editing (Manual vs Assistance only;
# Execution excluded because direct editing was structurally unavailable)
edit_ma <- intervention_types_raw |> dplyr::filter(mode != "Execution")
edit_table_ma <- table(edit_ma$mode, edit_ma$had_edit)
fisher_edit <- fisher.test(edit_table_ma)

fisher_types_table <- tibble::tibble(
  `Behavioral Category`  = c("Direct Editing (Manual vs. Assistance only)", "Suggestion Rejection", "Override"),
  `Fisher's p-value`     = c(
    ifelse(fisher_edit$p.value    < .001, "< .001", round(fisher_edit$p.value,     3)),
...
knitr::kable(
  fisher_types_table,
  caption = "Fisher's exact tests for behavioral intervention type occurrence. Direct Editing was tested only between the Manual and Assistance conditions, as it was structurally unavailable in the Execution condition.",
  align = c("l", "c")
)
```
